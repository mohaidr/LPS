...
namespace LPS.Infrastructure.Monitoring.EventSources
{
    ...
    public class LPSRequestEventSource : EventSource
    {
        private static readonly ConcurrentDictionary<LPSHttpRun, LPSRequestEventSource> instances = new ConcurrentDictionary<LPSHttpRun, LPSRequestEventSource>();

        private IncrementingEventCounter requestIncrementCounter;

        private LPSRequestEventSource(LPSHttpRun lpshttpRun)
        {
        ...
        public static LPSRequestEventSource GetInstance(LPSHttpRun lpsHttpRun)
        {
            return instances.GetOrAdd(lpsHttpRun, (run) => new LPSRequestEventSource(run));
        }
