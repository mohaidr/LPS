...
namespace LPS.Infrastructure.PlaceHolderService.Methods
{
    ...
    {
        public CeilMethod(ParameterExtractorService p, ILogger l, IRuntimeOperationIdProvider op, IVariableManager v, Lazy<IPlaceholderResolverService> r, ISessionManager sessionManager)
        ...
        public override async Task<string> ExecuteAsync(string parameters, string sessionId, CancellationToken token)
        {
            ...
            try
            {
                ...
                if (value is null)
                    return await FailNumberAsync(variableName, "ceil", token, "A numeric 'source' is required.", sessionId: sessionId, isGlobal: isGlobal);

                return await StoreNumberResultAsync(variableName, Math.Ceiling(value.Value), asType, token, sessionId: sessionId, isGlobal: isGlobal);
            }
            catch (Exception ex)
            {
