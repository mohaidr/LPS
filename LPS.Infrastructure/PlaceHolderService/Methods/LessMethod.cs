...
namespace LPS.Infrastructure.PlaceHolderService.Methods
{
    ...
    {
        public LessMethod(ParameterExtractorService p, ILogger l, IRuntimeOperationIdProvider op, IVariableManager v, Lazy<IPlaceholderResolverService> r, ISessionManager sessionManager)
        ...
        public override async Task<string> ExecuteAsync(string parameters, string sessionId, CancellationToken token)
        {
            ...
            try
            {
                ...
                if (a is null || b is null)
                    return await FailNumberAsync(variableName, "less", token, "Both numeric operands 'a' and 'b' are required.", sessionId: sessionId, isGlobal: isGlobal);

                return await StoreNumberResultAsync(variableName, Math.Min(a.Value, b.Value), asType, token, sessionId: sessionId, isGlobal: isGlobal);
            }
            catch (Exception ex)
            {
