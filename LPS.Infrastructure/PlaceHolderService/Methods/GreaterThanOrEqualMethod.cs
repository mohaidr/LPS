...
{
    /// <summary>$greaterthanorequal(a=..., b=..., variable=name) — returns "true" when a &gt;= b, else "false".</summary>
    public sealed class GreaterThanOrEqualMethod : MethodBase
    {
        public GreaterThanOrEqualMethod(ParameterExtractorService p, ILogger l, IRuntimeOperationIdProvider op, IVariableManager v, Lazy<IPlaceholderResolverService> r, ISessionManager sessionManager)
        ...
        public override async Task<string> ExecuteAsync(string parameters, string sessionId, CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    await _logger.LogAsync(_op.OperationId, "greaterthanorequal failed. Both numeric operands 'a' and 'b' are required.", LPSLoggingLevel.Warning, token);
            ...
            {
                await _logger.LogAsync(_op.OperationId, $"greaterthanorequal failed. {ex}", LPSLoggingLevel.Error, token);
                await StoreStringVariableAsync(variableName, "false", token, sessionId, isGlobal);
                return "false";
            }
        }
